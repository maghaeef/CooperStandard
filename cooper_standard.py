...
#     def generate_synthetic_minority_data(self, lb, ub, num_low=10, num_high=10, ts_length=300):
#         """
#         Generate synthetic data for minority classes (t5 values below lb and above ub).
        
#         Parameters:
#             lb (float): Lower bound for t5.
#             ub (float): Upper bound for t5.
#             num_low (int): Number of synthetic samples to generate for t5 below lb.
#             num_high (int): Number of synthetic samples to generate for t5 above ub.
#             ts_length (int): Length of the synthetic time series.
        
#         Returns:
#             pd.DataFrame: A DataFrame containing synthetic rows that match the expected columns.
#         """
        
#         synthetic_data = []
#         # Define a fixed start_time for synthetic data (adjust as needed)
#         base_start_time = pd.Timestamp("2025-01-01 08:00:00")
        
#         # Function to generate synthetic time series data as a list of [time, value] pairs
#         def create_synthetic_ts(length):
#             times = list(np.linspace(0, length, length))
#             # Generate synthetic measurements (you can adjust the ranges if desired)
#             values = list(np.random.uniform(0, 100, length))
#             return [[t, v] for t, v in zip(times, values)]
        
#         # Generate synthetic rows for the "low" t5 region (t5 < lb)
#         for i in range(num_low):
#             batch_number = f"synthetic_low_{i+1}"
#             # Generate scalar features (adjust ranges as appropriate)
#             mh = random.uniform(50, 150)
#             ml = random.uniform(10, 50)
#             TimeAtML = random.uniform(0, 100)
#             TimeAtML_min = random.uniform(0, 50)
#             ml_min = random.uniform(5, 25)
#             # For "end-start", choose a value between 300 and 600 seconds
#             duration = random.uniform(300, 600)
#             start_time = base_start_time
#             end_time = start_time + pd.Timedelta(seconds=duration)
#             # t5 for low samples: choose a value clearly below lb
#             t5 = random.uniform(lb - 10, lb - 1)
#             # Create synthetic time series for MDRTorqueS1 and MDRTorqueS2
#             ts_s1 = create_synthetic_ts(ts_length)
#             ts_s2 = create_synthetic_ts(ts_length)
...
#         # Generate synthetic rows for the "high" t5 region (t5 > ub)
#         for i in range(num_high):
#             batch_number = f"synthetic_high_{i+1}"
#             mh = random.uniform(50, 150)
#             ml = random.uniform(10, 50)
#             TimeAtML = random.uniform(0, 100)
#             TimeAtML_min = random.uniform(0, 50)
...
#             start_time = base_start_time
#             end_time = start_time + pd.Timedelta(seconds=duration)
#             # t5 for high samples: choose a value clearly above ub
#             t5 = random.uniform(ub + 1, ub + 10)
#             ts_s1 = create_synthetic_ts(ts_length)
#             ts_s2 = create_synthetic_ts(ts_length)
